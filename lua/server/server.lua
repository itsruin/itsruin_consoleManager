...
function add(type, user_id, value, amount)
    ...
    if type == 'cash' then
        ...
        print(string.format('[ITSRUIN_CM] [+] cash added: %s (%d번), %s원', name, user_id, tostring(comma_value(value))))

    elseif type == 'bank' then
        vRP.giveBankMoney({user_id, value})
        print(string.format('[ITSRUIN_CM] [+] bank money added: %s (%d번), %s원', name, user_id, tostring(comma_value(value))))

    elseif type == 'item' then
        vRP.giveInventoryItem({user_id, value, amount, true})
        print(string.format('[ITSRUIN_CM] [+] item added: %s (%d번), %s (%s개)', name, user_id, value, tostring(comma_value(amount))))
    ...
    elseif type == 'vehicle' then
        ...
        print(string.format('[ITSRUIN_CM] [+] vehicle added: %s (%d번), %s', name, user_id, value))

    elseif type == 'group' then
        vRP.addUserGroup({user_id, value})
        print(string.format('[ITSRUIN_CM] [+] group added: %s (%d번), %s', name, user_id, value))

    else
        print(string.format('[ITSRUIN_CM] [!] 지정되지 않은 명령어입니다: %s', type))
    end
end
...
function remove(type, user_id, value, amount)
    ...
    if type == 'cash' then
        ...
        print(string.format('[ITSRUIN_CM] [-] cash removed: %s (%d번), %s원', name, user_id, tostring(comma_value(value))))

    elseif type == 'bank' then
        local currentValue = vRP.getBankMoney({user_id})
        vRP.setBankMoney({user_id, currentValue - value})
        print(string.format('[ITSRUIN_CM] [-] bank money removed: %s (%d번), %s원', name, user_id, tostring(comma_value(value))))

    elseif type == 'item' then
        vRP.tryGetInventoryItem({user_id, value, amount, true})
        print(string.format('[ITSRUIN_CM] [-] item removed: %s (%d번), %s (%s개)', name, user_id, value, tostring(comma_value(amount))))
    ...
    elseif type == 'vehicle' then
        ...
        print(string.format('[ITSRUIN_CM] [-] vehicle removed: %s (%d번), %s', name, user_id, value))

    elseif type == 'group' then
        vRP.removeUserGroup({user_id, value})
        print(string.format('[ITSRUIN_CM] [-] group removed: %s (%d번), %s', name, user_id, value))

    else
        print('[ITSRUIN_CM] [!] 지정되지 않은 명령어입니다.')
    end
end
...
function reset(type, user_id)
    ...
    if type == 'cash' then
        ...
        print(string.format('[ITSRUIN_CM] [/] cash reset: %s (%d번)', name, user_id))

    elseif type == 'bank' then
        vRP.setBankMoney({user_id, 0})
        vRPclient.notify(source, {'시스템 관리자에 의해 모든 계좌 잔액이 회수되었습니다.'})
        print(string.format('[ITSRUIN_CM] [/] bank money reset: %s (%d번)', name, user_id))

    elseif type == 'item' then
        vRP.clearInventory({user_id})
        vRPclient.notify(source, {'시스템 관리자에 의해 모든 아이템이 회수되었습니다.'})
        print(string.format('[ITSRUIN_CM] [/] inventory reset: %s (%d번)', name, user_id))
    ...
    elseif type == 'vehicle' then
        ...
        print(string.format('[ITSRUIN_CM] [/] vehicle reset: %s (%d번)', name, user_id))
    ...
    elseif type == 'newbiecode' then
        ...
        print(string.format('[ITSRUIN_CM] [/] database - ' .. database.newbie .. ', reset: %s (%d번)', name, user_id))
    ...
    elseif type == 'newbiecode-discord' then
        ...
        print(string.format('[ITSRUIN_CM] [/] database - ' .. database.newbie .. ', state = 1: %s (%d번)', name, user_id))
    ...
    elseif type == 'newbiecode-reward' then
        ...
        print(string.format('[ITSRUIN_CM] [/] database - ' .. database.newbie .. ', state = 2: %s (%d번)', name, user_id))
        ...
        if command == 'add' then
            if #args == 4 or #args == 5 then
                ...
                if user_id then
                    ...
                    print('[ITSRUIN_CM] [!] 대상자가 접속 중이지 않거나 값이 잘못되었습니다.')
                end
            end
